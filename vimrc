...
" TODO: FUTURE " NeoBundle 'vim-multiple-cursors'
  ...
  "
    ...
    let g:neosnippet#snippets_directory='~/.vim/neosnippets/ruby/'
...
" migrationfile2xls " :%s/ *\(t\.\|:\|limit *=> *\)/\t/gc
" migration2yml " :%s/',.*comment: '*/: /gc | %s/'$//gc | %s/^.* ://gc
" Gemfile2xls " %s/\n//gc | %s/gem ["']/\tgem '/gc | %s/['"]#/'\r#/gc | M/gem
" mysqldesc2seed.csv: " :'<,'>s/  *.*\n/,/gc
" function rubyIntoRocketHash() :%s/:\(.*\)/\1:/gc
" #from HTML into seleniumIDE " :%s/\(.quot\;\)*<.td>.*$/",/gc | %s/^.*<td>/"/gc
"
" manual-merge
