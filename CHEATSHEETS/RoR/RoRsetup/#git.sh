#git-setup
	...
	$ git config --global user.email some_body@dena.com
	$ git config --global core.editor 'vim -c "set fenc=utf-8"'
	$ git config --global user.name "some_body"
	Mac上のGit設定
		$ ssh-keygen -t rsa -C "some_body@dena.com"
		$ pbcopy ~/.ssh/id_rsa.pub
	#agent転送でpassword 入力省略
	  eval `ssh-agent -s`
  ssh-add ~/.ssh/id_rsa
 ssh-add -l
	#TEST
		$  ssh -T git@github.dena.jp
		#type yes, and type fingerprint password
			"Hi some_body! You've successfully authenticated, but GitHub does not provide shell access."
	#管理者に上の公開鍵連絡送付
...
$ git config -l
	#post-merge cleaning 
		#コメント文点検 
		:vimgrep /\(context\|it \)/gc % 
		rubocop; rspec 
		#rubocop で検出されない \s{2,}[=?]\s{2,}を除去 
		#git diff /  \w で  を検索するのがはやい 
			{2}(!|=|&&|\|\||\?)+ {2} 
		rspec integration/ 
	 
	**git rebaseルール**
	git pull と pull request 時のルール

以下のようにルールを定めます。
master を git pull する時は rebase する

ローカル環境の master ブランチを最新状態にするために git pull しますが、そのとき --rebase オプションを付けてローカルの修正はリベースした状態でリモートの master 修正をローカルに取り込みます。

$ git pull --rebase

毎回打つのは面倒なので、下記コマンドでデフォルトの設定を変更します。
